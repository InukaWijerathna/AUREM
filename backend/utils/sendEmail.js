...
const sendEmail = async ({ to, subject, html }) => {
  ...
  return transporter.sendMail({
    from: `"${process.env.FROM_NAME || 'EMarket'}" <${process.env.FROM_EMAIL || process.env.SMTP_USER}>`,
    to,
    subject,
    html,
  });
...
const emailTemplates = {
  verification: (name, url) => `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #4F46E5;">Verify your EMarket account</h2>
      <p>Hi ${name}, thanks for signing up!</p>
      <p>Click the button below to verify your email address:</p>
      <a href="${url}" style="display:inline-block;padding:12px 24px;background:#4F46E5;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold;">Verify Email</a>
  ...
  orderConfirmation: (name, orderId, total) => `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>We'll notify you when your order ships. Thank you for shopping with EMarket!</p>
    </div>
  `,
  orderShipped: (name, orderId, tracking) => `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Thank you for shopping with EMarket!</p>
    </div>
  `,
};
